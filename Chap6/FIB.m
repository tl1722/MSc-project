function w = FIB(noTap,micNum,lkDir,f_low,f_high,f_step,ref_omega,fs,x_array,d,c)
%% Design of  FIB 

% Allocating arrays
s1=[]; sx2=[];s_sum_total =zeros(micNum*noTap,micNum*noTap);S1_omega_theta_ref = zeros(micNum*noTap,micNum*noTap);
...
%% steering vector and big S for reference frequency for mainlobe
...
% steering vectors for angles less than lkDir:
...
%% Calculation of Q matrix and 'a' matrix and weights

alpha = 0.01;  % weighting parameter

beta = 0.95;  % control parameter

Q = S_sum_ref + alpha.*(S1_omega_theta_ref)  + beta.*s_sum_total; %eqn (13)

a = (sum(s_sum_ref,2));           % eqn (14)
