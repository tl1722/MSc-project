...
rir_diffuse = rir_generator(c, fs, position_mics, position_source, room, 0, n);
R=rir_diffuse*rir_diffuse';
...
%    w_lcmv=ones(num_mics,length(freqVec));
...
for i=1:num_mics
  ...
  for n=1:n_frame
      % mvdr
%       beam_frame(n,:,i)=w_mvdr(i,:).*(frame_received(n,:));
